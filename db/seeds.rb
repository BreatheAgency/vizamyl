...
I18n.available_locales.each do |locale|
  ...
  [true, false].each do |super_user|
    user = User.new(locale: locale, invite_code: 'invite', title: 'Ms', first_name: Faker::Name.first_name, last_name: Faker::Name.last_name, email: "#{locale}-#{(super_user) ? 'super-' : ''}user@example.com", password: 'password', password_confirmation: 'password', super_user: super_user)
    user.save!
