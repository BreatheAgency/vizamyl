...
I18n.available_locales.each do |locale|
  ...
  [true, false].each do |super_user|
    user = User.new(locale: locale, invite_code: 'vzmyl100', title: 'Dr', first_name: Faker::Name.first_name, last_name: Faker::Name.last_name, email: "#{locale}-#{(super_user) ? 'super-' : ''}user@readvizamyl.com", password: 'vzmyl100', password_confirmation: 'vzmyl100', super_user: super_user)
    user.save!
...
qr = QuestionRound.find(96)
(1..15).each do |i|
  q = qr.questions.create!(title: "Case #{i}", test_case: true)
