class CourseCompletionLetter < Prawn::Document
  def initialize(user, from_active_admin = false)
    ...
    font('geinspira')
    image(StringIO.new(File.binread(Rails.root.join("app/assets/images/course-completion-#{I18n.locale}-updated.jpg"))), at: [bounds.absolute_left, PDF::Core::PageGeometry::SIZES['LETTER'][1] - bounds.absolute_bottom], fit: PDF::Core::PageGeometry::SIZES['LETTER'])
    left = 68
    top = 415
    height = 31.5
    margin = 2
...
class CourseCompletionA4 < Prawn::Document
  def initialize(user, from_active_admin = false)
    ...
    font('geinspira')
    image(StringIO.new(File.binread(Rails.root.join("app/assets/images/course-completion-#{I18n.locale}-updated.jpg"))), at: [bounds.absolute_left, PDF::Core::PageGeometry::SIZES['A4'][1] - bounds.absolute_bottom], fit: PDF::Core::PageGeometry::SIZES['A4'])
    left = 68
...
class CourseCompletionA4WithDepartment < Prawn::Document
  def initialize(user, from_active_admin = false)
    ...
    # Labels from translation + values — clean JPG template (no pre-printed labels)
    text_box(I18n.t('course_completion.institution') + ':', at: [left, top + (margin*5) + (height*5)], width: label_width, style: :normal, size: 14)
    text_box(user.institution, at: [left + label_width, top + (margin*5) + (height*5)], style: :normal, size: 14)
    text_box(I18n.t('course_completion.department') + ':', at: [left, top + (margin*4) + (height*4)], width: label_width, style: :normal, size: 14)
    text_box(user.department.to_s, at: [left + label_width, top + (margin*4) + (height*4)], style: :normal, size: 14)
    text_box(I18n.t('course_completion.last_name') + ':', at: [left, top + (margin*3) + (height*3)], width: label_width, style: :normal, size: 14)
    text_box(user.last_name, at: [left + label_width, top + (margin*3) + (height*3)], style: :normal, size: 14)
    text_box(I18n.t('course_completion.first_name') + ':', at: [left, top + (margin*2) + (height*2)], width: label_width, style: :normal, size: 14)
    text_box(user.first_name, at: [left + label_width, top + (margin*2) + (height*2)], style: :normal, size: 14)
    completion_date = from_active_admin ? user.passed_round_two_at : Time.now
    completion_date ||= Time.now
    date_format = (user.locale == 'en-us') ? "%m/%d/%Y" : "%d/%m/%Y"
    text_box(I18n.t('course_completion.date') + ':', at: [left, top + margin + height], width: label_width, style: :normal, size: 14)
    text_box(I18n.l(completion_date, locale: user.locale, format: date_format), at: [left + label_width, top + margin + height], style: :normal, size: 14)
  end
end
