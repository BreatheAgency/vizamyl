Course.ProgressionController = Ember.Controller.extend({
  ...
  total: function() {
    ...
    var total = Math.round((this.get('completeProgressions.length') - this.get('hiddenChapters.lastObject.steps.length')) / (this.get('progressions.length') - this.get('hiddenChapters.lastObject.steps.length')) * 100) || 0;
    return total;
